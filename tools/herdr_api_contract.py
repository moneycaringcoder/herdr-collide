#!/usr/bin/env python3
"""Verify a Herdr API schema still carries everything collide depends on.
...
# Request methods collide calls, with the parameters it sends.
#
# "required" means collide always sends the field, so the schema must still
# accept it and still list it as required. "optional" means collide sends it
...
# Response variants collide reads, keyed by the `type` discriminant, with the
...
#
# "required" means collide treats absence as breakage rather than as an empty
# result. "optional" means collide reads it when present and copes when it is
# not, so the field must still exist but need not be mandatory.
OBJECTS: dict[str, dict[str, tuple[str, ...]]] = {
    "SessionSnapshot": {
        # `workspaces` absent is a protocol break rather than an idle session,
        # and the client says so explicitly rather than returning nothing.
...
# that means the configuration took effect; the other two are reported as
# errors carrying their diagnostics.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check Herdr's API against collide's needs")
